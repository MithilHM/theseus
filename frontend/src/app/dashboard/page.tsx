...
export default function DashboardPage() {
  ...
  const liveSummary = {
    currentCashBalance: summary?.cash_balance ?? 435000,
    netCashFlow: summary?.net_cash_flow ?? 45000,
    burnRate: summary?.burn_rate ?? 12000,
    cashRunwayMonths: summary?.runway_days ? (summary.runway_days / 30) : 2.5,
    liquidityScore: summary?.liquidity_score ?? 72,
  ...
  const liveCashForecast = [
    ...
    { date: '30 Days', p10: forecast30?.p10 ?? 380000, p50: forecast30?.p50 ?? 410000, p90: forecast30?.p90 ?? 440000 },
    { date: '60 Days', p10: forecast60?.p10 ?? 370000, p50: forecast60?.p50 ?? 415000, p90: forecast60?.p90 ?? 460000 },
    { date: '90 Days', p10: forecast90?.p10 ?? 355000, p50: forecast90?.p50 ?? 425000, p90: forecast90?.p90 ?? 495000 }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-8 space-y-8 relative">
        ...
        {/* Footer */}
        <footer className="pt-8 border-t border-slate-805 text-center flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-slate-500 font-medium">
          ...
          <div className="flex gap-4">
            ...
            <span>•</span>
            <span className="text-slate-600">System Status: Active</span>
          </div>
        </footer>
